...
class TenantConnectionService {
    ...
    async open() {
        await this.tenantConnection.initialize()
        if (this.tenantConnection.isInitialized) return
        await this.tenantConnection.destroy()
    }

    useManager() {
        // `this.managerRef` is EntityManager → `.connection` → `.dataSource`
        return this.managerRef.dataSource
    }

    useGetter() {
        // `this.entityManager` resolves via the getter's return type.
        return this.entityManager.dataSource.createQueryBuilder()
    }
}
