...
- 同时支持 [DataMapper](./guides/1-active-record-data-mapper.md#what-is-the-data-mapper-pattern) 和 [ActiveRecord](./guides/1-active-record-data-mapper.md#what-is-the-active-record-pattern)（由你选择）。
- 实体和列。
- 数据库特定的列类型。
- 实体管理器。
- 仓库和自定义仓库。
...
- 索引。
- 事务。
- 带自动生成的 [迁移](/docs/migrations/why)。
- 连接池。
- 复制。
- 使用多个数据库实例。
- 处理多种数据库类型。
- 跨数据库和跨 schema 查询。
- 优雅语法、灵活且强大的 QueryBuilder。
- 左连接和内连接。
- 使用连接进行查询时的正确分页。
- 查询缓存。
- 原始结果流式传输。
...
- 运行于 Node.js / Browser / Ionic / Cordova / React Native / NativeScript / Expo / Electron 平台。
